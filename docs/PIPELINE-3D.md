...
Compressão no cliente (≈ 2048px, JPEG 0,85) → upload para `captures/{jobId}/` com o SDK web do Storage.
...
- O painel faz **polling a cada 5 s** (com backoff até 30 s) enquanto houver job em `processing`, e também escuta o doc do produto com `onSnapshot`.
- **Idempotência:** o `GET` usa uma transação para que só uma requisição faça a cópia para o Storage (flag `job.finalizing`).
- **Fase 2:** trocar o polling por webhook do provedor, se disponível, ou por uma tarefa agendada.
...
export interface ModelProvider {
  name: "meshy" | "tripo";
  createTask(input: { imageUrls: string[]; productHint?: string }): Promise<{ taskId: string }>;
...
Meshy: 1 foto → endpoint *Image to 3D*; 2 a 4 fotos → *Multi-Image to 3D*. Pedir textura PBR, saída GLB e USDZ, e um número de polígonos moderado (o foco é abrir leve no celular).
